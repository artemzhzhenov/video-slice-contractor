...
1. **Мячик целиком в кадре.** `PROP_A` (радиус 2,8 см, на тонком штырьке) стоит у правого края кадра на
   ...
   кадре. **Штырёк в кадре виден** — он идёт от мячика вниз и уходит за нижний край (исправлено 21.09:
   в первой редакции отчёта ошибочно было написано «в кадре только мячик»).
   **Взгляд (исправлено 21.09):** первая редакция указывала промах 0,2° — это замер по одному левому
   глазу. По обоим глазам на кадре 1024: левый 0,2°, правый 9,9°, из точки между глаз 5,3°; на 1027 —
   2,3° / 11,5° / 7,0°. Глаза поворачиваются параллельно (один `gaze_yaw`/`gaze_pitch` на оба), а мячик
   в ~30 см от лица требует сведения ~10°, так что оба глаза одновременно в него не попадут; правильная
   цель — точка между глаз, тогда каждый глаз промахивается симметрично на ~5°. Применяю с SHOT_002;
   для SHOT_001 — по решению владельца (см. сопроводительное сообщение).
