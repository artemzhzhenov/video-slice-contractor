...
| `measure_hand_over_head.py` | **How much of the head the hand and the forearm cover** — ACCEPTANCE item 4 of burst 2 (a fifth to a half, `state_requirements.json → hand_over_head`), measured on a split render: the head re-projected from the exports (HEAD_HOLDOUT is cut by the forearm) against the forearm (object cryptomatte) and the hand (FOREGROUND_PLATE). Informs the owner, who judges the gesture; exit 0 either way. Bounding boxes said 28 % on the SHOT_003 early checkpoint where the render shows 16 % (2026-09-25). |
