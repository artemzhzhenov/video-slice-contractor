...
- **D7 round-trip on the real head (contractor v01, 2026-09-16)**, SHOT_001 at 100 % / 64 spp on
  ...
  - **Found: an open jaw fails a correct export.** Frame 1100 (jaw_open 0.87, mouth corners 0.96)
    ...
    recommended fix is to re-project the per-frame deformed default head. **OPEN, and it must land
    before burst 2 is accepted**: the laughter window in SHOT_001 opens the jaw. The
    `deformed_beyond_gate_calibration` proxy is also weak on the real head: it flags a blink (1.0,
    frame 1080) and mouth corners (0.74, the still) that leave the silhouette unchanged.
