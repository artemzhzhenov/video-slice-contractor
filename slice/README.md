...
| `check_asset.sh` | One command for a delivered scene: check_scene → export → check_exports → check_alembic → smoke render of the video frames `pick_frames.py` chooses and the still (64 spp / 25 %, the criterion-3 gate needs ≥ 64 spp) → split → composite → round-trip; stops at the first failing gate. The contractor's self-check and our acceptance (`contractor/burst-1/ACCEPTANCE.md`). |
