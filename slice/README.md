...
| `check_hand_pose.py` | **The hand-over-the-face pose gated on the shot file** (ACCEPTANCE item 4, `state_requirements.json → hand_pose`; owner 2026-09-26, option 1: the hand from below and in front, the elbow down at chest height, the palm on the eyes): per frame of the window, from the rig — the elbow no higher than the shoulder joint, the elbow 30–180°, the wrist within 50°, the forearm segment outside the head skin; from flat renders — no hand pixel behind the head or the hair (the pixels the foreground plate would lose), 5 mm of clearance from the skin. A rig without the arm chain reports the joints NOT_APPLICABLE, never a pass. SHOT_003 v02 checkpoint 2 solved the pose against the eye alone: elbow 206 mm above the shoulder, wrist 85°, forearm through the head. |
| `measure_hand_over_head.py` | **How much of the head the hand and the forearm cover** — ACCEPTANCE item 4 of burst 2 (the hand over the eyes; 0.1–0.5 of the head silhouette on every frame, `state_requirements.json → hand_over_head` — the task's eyeball "a fifth" invited solving the number instead of the gesture — `measurements/hand_over_head_attainable_2026-09-26.json`; the anatomy is gated by `check_hand_pose.py`), measured on a split render: the head re-projected from the exports (HEAD_HOLDOUT is cut by the forearm) against the forearm (object cryptomatte) and the hand (FOREGROUND_PLATE). In range only when every measured frame is (the hold, Q40 — not the peak); names the frames outside it and the lowest. Informs the owner, who judges the gesture; exit 0 either way. Bounding boxes said 28 % on the SHOT_003 early checkpoint where the render shows 16 % (2026-09-25). |
